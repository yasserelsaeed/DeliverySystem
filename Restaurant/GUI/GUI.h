...
private:
	...
	static const int
		WindWidth = 1500, WindHeight = 800,		//Window width and height
		StatusBarHeight = 300,	//Status Bar Height
		MenuBarHeight = 80,		//Menu Bar Height (distance from top of window to bottom line of menu bar)
		MenuItemWidth = 100,		//Width of each item in menu bar menu
...
public:
	...
	void PrintRegInfo(int*, int*, int*, int* ,string);							//Prints all information about the region
	void PrintServedOrders(int*servA, int*servB, int*servC, int*servD);			//Prints the number of served orders of each type in each region
	void CallPrintAssignedMotorcycleToOrder(Queue<Motorcycle*>&, Queue<Motorcycle*>&, Queue<Motorcycle*>&, Queue<Motorcycle*>&);									//added
																				//Calls a function the prints the assigned motorcycles
	void CallPrintReturnedMotorcycles(Queue<Motorcycle*>&, Queue<Motorcycle*>&, Queue<Motorcycle*>&, Queue<Motorcycle*>&);
																				////Calls a function the prints the returned motorcycles
	void PrintAssignedMotorcycleToOrder(Motorcycle*& Mptr,int x,int y);			//Prints the assigned motorcycles
	void PrintReturnedMotorcycle(Motorcycle*& Mptr, int x, int y);				//Prints the returned motorcycles
	...
	void RemoveFromOrdersArray(Order* CanceledOrder);							//removes an order from the drawing list
